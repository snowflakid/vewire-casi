...
const Sidebar = ({ isOpen = false, activeGame, setGame, onOpenProfile, onOpenLeaderboard, onOpenSettings }: SidebarProps) => {
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-20 md:w-64 ${THEME.sidebar} border-r ${THEME.border} flex flex-col transition-all ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
      <div className="h-20 flex items-center justify-center md:justify-start md:px-6 border-b border-vewire-border">
      ...
      <div className="p-4 flex flex-col gap-2 overflow-y-auto">
        <div className="text-xs font-bold text-gray-500 uppercase px-4 mb-2 hidden md:block">{t('nav.games')}</div>
        {GAMES_CONFIG.map(g => (
          <button
            key={g.id}
            onClick={() => handleGameSelect(g.id)}
            className={`flex items-center gap-4 p-3 rounded-lg transition-all ${activeGame === g.id ? 'bg-vewire-card text-white' : 'text-gray-400 hover:text-white hover:bg-vewire-card/50'}`}
          >
            <g.icon size={20} className={activeGame === g.id ? g.color : ''} />
            <span className="font-bold hidden md:block">{g.name}</span>
            {activeGame === g.id && <div className="ml-auto w-2 h-2 rounded-full bg-vewire-accent hidden md:block" />}
          </button>
        ))}
      </div>
      ...
      <div className="mt-auto p-4 border-t border-vewire-border">
        ...
        <button onClick={() => handleMenuClick(onOpenProfile)} className="flex items-center gap-4 p-3 text-gray-400 hover:text-white w-full">
            <User size={20} />
            <span className="hidden md:block font-medium">{t('nav.profile')}</span>
        </button>
        <button className="flex items-center gap-4 p-3 text-gray-400 hover:text-white w-full">
            <ShieldCheck size={20} />
            <span className="hidden md:block font-medium">{t('nav.fairness')}</span>
        </button>
        <button onClick={() => handleMenuClick(onOpenSettings)} className="flex items-center gap-4 p-3 text-gray-400 hover:text-white w-full">
            <Settings size={20} />
